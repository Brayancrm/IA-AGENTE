...
const DashboardWithFirebase = ({
  ...
  const tvStockAlerts = useMemo(() => {
    if (!user?.isMaster || !catalogItems?.length) return [];
    const alerts = [];
  ...
  const markTvLoginAsSold = async (item) => {
    ...
    if (!window.confirm(t('tvLogins.confirmMarkSold'))) return;
    try {
  ...
  const resendTvCredentials = async (item) => {
    if (!user?.uid || !item?.id) return;
    try {
      const response = await fetch(`${BACKEND_URL}/api/tv/resend-credentials`, {
        method: 'POST',
  ...
  const loadOrderReports = async () => {
    ...
    try {
      const q = new URLSearchParams({
        userId: user.uid,
        start: reportsRange.start || '2000-01-01',
        end: reportsRange.end || '2099-12-31'
      });
      const response = await fetch(`${BACKEND_URL}/api/orders/summary?${q}`);
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Erro');
      setReportsData(data);
    } catch (e) {
  ...
  const deleteTvLogin = async (item) => {
    if (!database || !user?.uid || !item?.id) return;
    if (!window.confirm(t('tvLogins.confirmDelete'))) return;
    try {
      const itemRef = ref(database, `users/data/${user.uid}/tv_logins/${item.id}`);
  ...
  const renderContent = () => {
    ...
    switch (currentPage) {
        ...
        return (
          <div style={{ padding: getResponsivePadding(), width: '100%', boxSizing: 'border-box', overflowX: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'flex-end', marginBottom: '20px' }}>
              <div>
                <label htmlFor="rep-start" style={{ display: 'block', fontSize: '0.8rem', color: '#9ca3af', marginBottom: '4px' }}>{t('reportsPage.start')}</label>
                <input
                  id="rep-start"
                  type="date"
                  value={reportsRange.start}
                  onChange={(e) => setReportsRange((r) => ({ ...r, start: e.target.value }))}
                  style={{ padding: '10px', borderRadius: '8px', border: '1px solid #374151', background: '#0f1419', color: '#fff' }}
                />
              </div>
              <div>
                <label htmlFor="rep-end" style={{ display: 'block', fontSize: '0.8rem', color: '#9ca3af', marginBottom: '4px' }}>{t('reportsPage.end')}</label>
                <input
                  id="rep-end"
                  type="date"
                  value={reportsRange.end}
                  onChange={(e) => setReportsRange((r) => ({ ...r, end: e.target.value }))}
                  style={{ padding: '10px', borderRadius: '8px', border: '1px solid #374151', background: '#0f1419', color: '#fff' }}
                />
              </div>
              <button
                ...
                style={{
                  ...
                  color: '#fff',
                  fontWeight: 700,
                  cursor: reportsLoading ? 'wait' : 'pointer'
                }}
              >
                {reportsLoading ? '…' : t('reportsPage.load')}
              </button>
            </div>
            {reportsData ? (
              <div style={{ backgroundColor: '#1a1f36', borderRadius: '16px', padding: '20px', border: '1px solid rgba(16,185,129,0.2)' }}>
                <p style={{ color: '#e5e7eb', marginBottom: '8px' }}>
                  {t('reportsPage.count')}: <strong>{reportsData.count}</strong>
                </p>
                <p style={{ color: '#e5e7eb', marginBottom: '8px' }}>
                  {t('reportsPage.totalPaid')}: <strong>R$ {Number(reportsData.totalPaidValue || 0).toFixed(2)}</strong>
                </p>
                <p style={{ color: '#9ca3af', fontSize: '0.9rem', marginBottom: '16px' }}>
                  {t('reportsPage.byStatus')}: {JSON.stringify(reportsData.byStatus || {})}
                </p>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem', color: '#d1d5db' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid #374151' }}>
                        <th style={{ textAlign: 'left', padding: '8px' }}>{t('reportsPage.tableId')}</th>
                        <th style={{ textAlign: 'left', padding: '8px' }}>{t('reportsPage.tableStatus')}</th>
                        <th style={{ textAlign: 'right', padding: '8px' }}>{t('reportsPage.tableTotal')}</th>
                        <th style={{ textAlign: 'left', padding: '8px' }}>{t('reportsPage.tableDate')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(reportsData.orders || []).map((o) => (
                        <tr key={o.id} style={{ borderBottom: '1px solid rgba(55,65,81,0.5)' }}>
                          <td style={{ padding: '8px', wordBreak: 'break-all' }}>{o.id}</td>
                          <td style={{ padding: '8px' }}>{o.status}</td>
                          <td style={{ padding: '8px', textAlign: 'right' }}>{o.totalValue != null ? Number(o.totalValue).toFixed(2) : '—'}</td>
                          <td style={{ padding: '8px' }}>{o.createdAt ? new Date(o.createdAt).toLocaleString('pt-BR') : '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      case 'plans':
        return (
          <div style={{ padding: getResponsivePadding(), width: '100%', boxSizing: 'border-box', overflowX: 'hidden' }}>
            <div style={{ marginBottom: isMobile ? '14px' : '32px' }}>
              <h2 style={{ fontSize: isMobile ? '1.75rem' : '2.25rem', fontWeight: '700', color: '#ffffff', marginBottom: '8px', display: isMobile ? 'none' : 'flex', alignItems: 'center', gap: '12px' }}>
                {renderPageIcon('plans')}
                {t('plans.title')}
              </h2>
              <p style={{ fontSize: isMobile ? '0.78rem' : '1.125rem', color: '#9ca3af', lineHeight: 1.45 }}>
                {user?.isMaster ? t('plans.masterSubtitle') : t('plans.userSubtitle')}
              </p>
            </div>

            {user?.isMaster && (
              <div style={{ marginBottom: isMobile ? '14px' : '24px', display: 'flex', justifyContent: isMobile ? 'stretch' : 'flex-end' }}>
                <button
                ...
                {(user?.isMaster ? plans : plans.filter(p => p.active)).map((plan) => (
                  <div
                    ...
                    style={{
                      ...
                      border: plan.active
                        ? (isMobile ? '1px solid #10b981' : '2px solid #10b981')
                        : (isMobile ? '1px solid #4b5563' : '2px solid #4b5563'),
                      boxShadow: plan.active ? '0 4px 16px rgba(16, 185, 129, 0.15)' : '0 2px 10px rgba(0,0,0,0.2)',
                      transition: 'all 0.2s ease',
                      cursor: 'pointer',
                      position: 'relative',
                      maxWidth: '100%',
                  ...
                  >
                    ...
                    <div style={{ marginBottom: isMobile ? '10px' : '16px', position: 'relative', paddingRight: plan.active ? (isMobile ? '72px' : '80px') : '0' }}>
                      {/* Badge de Status */}
                      {plan.active && (
                    ...
                    border: '1px solid #10b981',
                      ...
                      <h3 style={{ fontSize: isMobile ? '1.05rem' : '1.75rem', fontWeight: '700', color: '#ffffff', marginBottom: '6px', wordBreak: 'break-word', lineHeight: 1.25 }}>
                        {plan.name}
                      </h3>
                      <p style={{ fontSize: isMobile ? '0.75rem' : '0.9375rem', color: '#9ca3af', wordBreak: 'break-word', lineHeight: 1.4 }}>
                        {plan.description || t('plans.noDescription')}
                      </p>
                    </div>

                    {/* Preço */}
                    <div style={{ marginBottom: isMobile ? '14px' : '24px', padding: isMobile ? '10px' : '16px', backgroundColor: plan.isTrialPlan ? 'rgba(245, 158, 11, 0.1)' : 'rgba(16, 185, 129, 0.1)', borderRadius: isMobile ? '10px' : '12px', border: `1px solid ${plan.isTrialPlan ? 'rgba(245, 158, 11, 0.3)' : 'rgba(16, 185, 129, 0.3)'}` }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', flexWrap: 'wrap' }}>
                        {plan.isTrialPlan ? (
                          <span style={{ fontSize: isMobile ? '1.2rem' : '2rem', fontWeight: '700', color: '#f59e0b' }}>
                            🎁 {t('plans.free')}
                          </span>
                        ) : (
                          <>
                        <span style={{ fontSize: isMobile ? '1.45rem' : '2.5rem', fontWeight: '700', color: '#10b981' }}>
                          {formatPlanPrice(plan)}
                        </span>
                        <span style={{ fontSize: isMobile ? '0.78rem' : '1rem', color: '#9ca3af' }}>
                      ...
                      {plan.isTrialPlan && (
                        <p style={{ fontSize: isMobile ? '0.72rem' : '0.875rem', color: '#f59e0b', marginTop: '6px', fontWeight: '600', lineHeight: 1.35 }}>
                    ...
                    {/* Limites */}
                    <div style={{ marginBottom: isMobile ? '14px' : '24px' }}>
                      <h4 style={{ fontSize: isMobile ? '0.8rem' : '1rem', fontWeight: '600', color: '#ffffff', marginBottom: isMobile ? '8px' : '12px' }}>
                        {t('plans.limitsTitle')}
                      </h4>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {plan.limits?.messagesPerMonth !== null && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#d1d5db', fontSize: isMobile ? '0.72rem' : '0.875rem', lineHeight: 1.35 }}>
                            <span>📨</span>
                    ...
                    {/* Ações */}
                    {user?.isMaster ? (
                      <div style={{ display: 'flex', gap: '8px', marginTop: isMobile ? '12px' : '24px', paddingTop: isMobile ? '12px' : '24px', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            openPlanModal(plan);
                    ...
                    border: '1px solid #10b981',
                      ...
                      <div style={{ marginTop: isMobile ? '12px' : '24px', paddingTop: isMobile ? '12px' : '24px', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                        {(() => {
  ...
  const menuItems = useMemo(
    () => [
      { id: 'dashboard', label: t('nav.dashboard'), icon: '🏠', badge: 0 },
      { id: 'tutorials', label: t('nav.tutorials'), icon: '📚', badge: 0 },
      { id: 'company', label: t('nav.company'), icon: '👤', badge: 0 },
      { id: 'catalog', label: t('nav.catalog'), icon: '📦', badge: 0 },
      { id: 'agendamentos', label: t('nav.agendamentos'), icon: '📅', badge: 0 },
      { id: 'conversas', label: t('nav.conversas'), icon: 'whatsapp', badge: 0 },
      { id: 'crm', label: t('nav.crm'), icon: 'target', badge: 0 },
      { id: 'integrations', label: t('nav.integrations'), icon: '⚙️', badge: 0 },
      { id: 'whatsapp', label: t('nav.whatsapp'), icon: '📱', badge: 0 },
      { id: 'assistant', label: t('nav.assistant'), icon: '🤖', badge: 0 },
      ...
      ...(user?.isMaster
        ? [
            { id: 'reports', label: t('nav.reports'), icon: '📊', badge: 0 },
            {
              ...
              label: t('nav.stripe'),
              icon: 'stripe',
              badge: stripeOps.pastDue > 0 ? Math.min(stripeOps.pastDue, 99) : 0,
              badgeTone: 'warning'
            },
  ...
  return (
        ...
        <div style={{ minHeight: '100vh', backgroundColor: '#0f1419', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif', overflowX: 'hidden', maxWidth: '100vw', display: 'flex' }}>
            ...
            >
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        background: isLocked
                          ? (isMobile ? 'rgba(100, 116, 139, 0.06)' : 'rgba(107, 114, 128, 0.1)')
                          : currentPage === item.id && isMobile
                            ? 'rgba(16, 185, 129, 0.1)'
                            : 'transparent',
                        color: isLocked
                          ? '#94a3b8'
                          : currentPage === item.id
                            ? (isMobile ? '#059669' : '#ffffff')
                            : isMobile
                              ? '#334155'
                              : '#d1d5db',
                      ...
                      onMouseLeave={(e) => {
                        if (isLocked) return;
                        if (currentPage === item.id && isMobile) {
                      ...
                      <span style={{ 
                        ...
                        {item.icon === 'whatsapp' ? (
                          ...
                          <span style={{ 
                            ...
                            justifyContent: 'center',
                            filter: isLocked 
                              ? (shouldBeColored ? 'opacity(0.4)' : (isMobile ? 'opacity(0.45)' : 'brightness(0) invert(1) opacity(0.4)'))
                              : (shouldBeColored ? 'none' : (isMobile ? 'none' : 'brightness(0) invert(1)')),
                            opacity: isLocked ? 0.4 : 1,
                            transition: 'all 0.2s ease',
                            visibility: 'visible',
                      ...
                      {!isLocked && (item.badge || 0) > 0 && (
                        <span
                          ...
                          style={{
                            ...
                            height: '20px',
                            borderRadius: '9999px',
                            backgroundColor: item.badgeTone === 'warning' ? '#f59e0b' : '#ef4444',
                            color: '#fff',
                            fontSize: '11px',
                            fontWeight: 700,
                            display: 'inline-flex',
              ...
              <div style={{ 
                padding: '16px',
                borderTop: isMobile ? '1px solid #e2e8f0' : '1px solid rgba(16, 185, 129, 0.1)',
                display: 'flex',
                flexDirection: 'column',
                gap: '12px',
                flexShrink: 0,
                ...
                <div style={{ padding: '0 12px 4px' }}>
                  ...
                  <select
                    ...
                    style={{
                      ...
                      padding: '10px 12px',
                      borderRadius: '8px',
                      border: isMobile ? '1px solid #e2e8f0' : '1px solid rgba(255,255,255,0.12)',
                      background: isMobile ? '#ffffff' : '#0f1419',
                ...
                <div style={{ 
                  ...
                  <div
                    ...
                    overflow: 'hidden',
                      ...
                      backgroundColor: isMobile ? '#f1f5f9' : '#1a1f36',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center'
                    }}
