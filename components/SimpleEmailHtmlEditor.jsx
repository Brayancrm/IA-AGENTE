  ...
  const onPickImportFile = async (file) => {
    ...
    try {
      if (lower.endsWith('.zip')) {
        ...
        setImportStatus('');
        const baseOk = data.assetBase
          ? `URLs: ${data.assetBase}`
          : 'AVISO: backend sem PUBLIC_SERVER_URL — imagens podem falhar no telemóvel';
        alert(
          `Importado com ${data.imageCount || 0} imagem(ns).\n${baseOk}\n\nImportante: grava o template (Atualizar) e lança uma campanha NOVA.\nO layout e o botão/link do BeeFree são mantidos.`
        );
        return;
      }
